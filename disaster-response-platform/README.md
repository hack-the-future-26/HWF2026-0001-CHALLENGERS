...
- 🏥 **Nearest shelter/hospital lookup** — haversine-distance ranked, returned instantly on
  report submission.
...
Every "nearest shelter/hospital" result now includes an estimated travel time, calculated
from distance and an assumed average response speed (25 km/h) plus an 8-minute dispatch
buffer. **This is a planning estimate, not live GPS tracking of a real vehicle** — there's
...
| GET    | `/api/nearest?lat=&lng=` | Nearest 3 shelters + hospitals to a point (each includes `distanceKm` and `etaMinutes`) |
...
## 📡 Works when there's no signal: offline queue

GPS itself doesn't need cell or WiFi signal — it uses satellites, so a phone can get a
location fix with zero network connectivity (it just takes longer). Submitting the report
still needs *some* internet to reach the server, though — no web app can get around that.

To handle the gap, every report submission that fails because the device is offline (or
the request can't reach the server) is automatically saved in the browser's local storage
instead of being lost. A banner shows "📥 N reports waiting to send." The app retries
automatically the moment `navigator.onLine` fires, and also every 20 seconds in the
background, so as soon as *any* connectivity returns — even briefly, even weak — the
queued reports go out and a confirmation toast appears. This works per-device: leave the
app open (or reopen it later) and it keeps trying on its own.

## 🧭 Turn-by-turn directions to shelters/hospitals

On the **Live Map** tab, pick a shelter or hospital from the dropdown and tap
**🧭 Get Directions**. The app:
1. Gets your current GPS location
2. Fetches a real driving route from OSRM (OpenStreetMap's free routing engine, no API
   key) and draws it on the map
3. Shows plain-language turn-by-turn steps ("Turn left onto MG Road", "Continue straight",
   etc.) translated into English/Hindi/Telugu, plus total distance and time
4. Always also shows an **Open in Google Maps** link — if OSRM's public server is
   unreachable or rate-limited, tap that instead for guaranteed directions
