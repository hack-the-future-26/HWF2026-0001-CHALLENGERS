...
// ================= Offline queue (for "no signal" areas) =================
// If the report can't reach the server (device is offline, or the request
// fails with a network-level error), it's saved on-device and automatically
// resent the moment any connection becomes available - even a brief or weak
// one later. GPS itself works without cell/WiFi signal; this queue solves
// the other half of the problem: actually delivering the report once you
// have *any* connectivity again.
const OFFLINE_QUEUE_KEY = "rescuelink_offline_queue";
...
function setOfflineQueue(queue) {
  try {
    ...
    console.error("Could not persist offline queue (storage full or unavailable):", e);
  }
  updateQueueBanner();
}

function queueReportOffline(payload) {
...
function updateQueueBanner() {
  ...
  if (count > 0) {
    ...
    banner.textContent = "📥 " + t("queue_pending_prefix") + " " + count + " " + (count === 1 ? t("queue_pending_one") : t("queue_pending_many"));
  } else {
...
async function flushOfflineQueue() {
  ...
  for (const item of queue) {
    try {
      ...
      if (res.ok) {
        ...
        // Server rejected it outright (e.g. validation error) - won't ever
        // succeed by retrying, so drop it rather than queue forever.
        console.error("Queued report rejected by server, dropping:", await res.text());
      }
    } catch (err) {
      // Still no real connectivity - keep it queued for the next attempt.
  ...
  if (sentCount > 0) {
    showToast("✅ " + sentCount + " " + t(sentCount === 1 ? "queue_sent_one" : "queue_sent_many"));
    refreshDashboard();
    loadMapData();
  }
}
...
setInterval(flushOfflineQueue, 20000); // periodic retry in case 'online' doesn't fire reliably
// Script runs at the end of <body>, so the DOM (including #queueBanner) is
// already parsed - call directly rather than waiting for DOMContentLoaded,
// which may have already fired by this point.
updateQueueBanner();
flushOfflineQueue();
...
document.getElementById("reportForm").addEventListener("submit", async (e) => {
  ...
  try {
    if (!navigator.onLine) {
      // Don't even attempt the request - device reports no connection at all.
      const localId = queueReportOffline(payload);
      showQueuedResult(localId);
      resetFormFields();
      return;
    }
    ...
    showResult(data.report, data.nearest);
    resetFormFields();
  } catch (err) {
    if (err instanceof TypeError) {
      // A TypeError from fetch means the request never reached the server
      // (no network, DNS failure, etc.) - a real validation error from our
      // own API would have been a normal JSON response, not a thrown
      // TypeError. Safe to queue for automatic retry.
      const localId = queueReportOffline(payload);
      showQueuedResult(localId);
      resetFormFields();
    } else {
...
function showQueuedResult(localId) {
  const card = document.getElementById("resultCard");
  card.style.display = "block";
  document.getElementById("severityBadgeWrap").innerHTML = `<span class="severity-badge queued-badge">📥 ${t("queued_title")}</span>`;
  document.getElementById("responseEtaWrap").innerHTML = "";
  document.getElementById("resultId").textContent = t("queued_desc");
  document.getElementById("locationNote").textContent = "";
  document.getElementById("nearestHelp").innerHTML = "";
  card.scrollIntoView({ behavior: "smooth", block: "nearest" });
}

function showResult(report, nearest) {
  const card = document.getElementById("resultCard");
  card.style.display = "block";
  ...
  // Prominent "estimated time for help to arrive" - this is what people are
  // usually most anxious to know. Shown separately from the shelter/hospital
  // breakdown below, right under the severity badge.
  const etaWrap = document.getElementById("responseEtaWrap");
  if (report.estimatedResponseEtaMinutes != null) {
    etaWrap.innerHTML = `<div class="eta-hero">🚑 ${t("est_help_arrival")}: <strong>~${report.estimatedResponseEtaMinutes} ${t("min_eta_short")}</strong></div>`;
  } else {
    etaWrap.innerHTML = "";
  }
  ...
  if (nearest && nearest.hospitals && nearest.hospitals.length) {
    const sourceTag =
      nearest.hospitalSource === "openstreetmap"
        ? ` <span class="live-tag">${t("live_data_tag")}</span>`
        : ` <span class="demo-tag">${t("demo_data_tag")}</span>`;
    html += `<div class="help-block"><h4>🏥 ${t("nearest_hospitals")}${sourceTag}</h4>`;
...
async function loadMapData() {
  const [sheltersRes, hospitalsRes, reportsRes] = await Promise.all([
...
function populateDirectionsDropdown() {
  const select = document.getElementById("directionsDestination");
  if (!select) return;
  const currentValue = select.value;
  select.innerHTML = `<option value="">${t("directions_choose")}</option>`;

  if (sheltersData.length) {
    const group = document.createElement("optgroup");
    group.label = "🏠 " + t("nearest_shelters");
    sheltersData.forEach((s) => {
      const opt = document.createElement("option");
      opt.value = `shelter:${s.lat}:${s.lng}`;
  ...
  if (hospitalsData.length) {
    const group = document.createElement("optgroup");
    group.label = "🏥 " + t("nearest_hospitals");
    hospitalsData.forEach((h) => {
      const opt = document.createElement("option");
      opt.value = `hospital:${h.lat}:${h.lng}`;
      opt.textContent = h.name;
      group.appendChild(opt);
    });
    select.appendChild(group);
  }
  select.value = currentValue; // preserve selection across refreshes if still valid
}
...
function redrawMap() {
  ...
  allReports.forEach((r) => {
    ...
    circleMarker(r.lat, r.lng, severityColor(r.severity.level), 10)
      .bindPopup(
        `<b>${r.severity.level} · ${r.severity.score}/10</b><br>${escapeHtml(r.text)}<br><i>Status: ${r.status}</i>${audioLink}`
      )
      .addTo(markersLayer);
  });
}
...
// Uses OSRM's free public routing API (no key needed) to draw a real route
// and generate simple turn-by-turn instructions. If that service is
// unreachable (offline, demo server down/rate-limited), we always still
// offer a Google Maps deep link as a reliable fallback.
let routeLayer = null;

const MANEUVER_PHRASES = {
  en: {
    depart: "Start heading out",
    turn_left: "Turn left",
    turn_right: "Turn right",
    turn_straight: "Continue straight",
    turn_slight_left: "Bear slightly left",
    turn_slight_right: "Bear slightly right",
    turn_sharp_left: "Make a sharp left turn",
    turn_sharp_right: "Make a sharp right turn",
    turn_uturn: "Make a U-turn",
    roundabout: "Enter the roundabout",
    merge: "Merge",
    fork: "Keep at the fork",
    "end of road": "Continue at the end of the road",
    arrive: "You have arrived at your destination",
    continue: "Continue",
    onto: "onto",
  },
  hi: {
    depart: "यात्रा शुरू करें",
    turn_left: "बाएं मुड़ें",
    turn_right: "दाएं मुड़ें",
    turn_straight: "सीधे जाते रहें",
    turn_slight_left: "थोड़ा बाएं मुड़ें",
    turn_slight_right: "थोड़ा दाएं मुड़ें",
    turn_sharp_left: "तेज़ी से बाएं मुड़ें",
    turn_sharp_right: "तेज़ी से दाएं मुड़ें",
    turn_uturn: "यू-टर्न लें",
    roundabout: "गोल चक्कर में प्रवेश करें",
    merge: "मिलाएं",
    fork: "मोड़ पर बने रहें",
    "end of road": "सड़क के अंत में आगे बढ़ें",
    arrive: "आप अपने गंतव्य पर पहुंच गए हैं",
    continue: "आगे बढ़ते रहें",
    onto: "पर",
  },
  te: {
    depart: "ప్రయాణం ప్రారంభించండి",
    turn_left: "ఎడమవైపు తిరగండి",
    turn_right: "కుడివైపు తిరగండి",
    turn_straight: "నేరుగా కొనసాగించండి",
    turn_slight_left: "కొద్దిగా ఎడమవైపు తిరగండి",
    turn_slight_right: "కొద్దిగా కుడివైపు తిరగండి",
    turn_sharp_left: "పదునైన ఎడమ మలుపు తీసుకోండి",
    turn_sharp_right: "పదునైన కుడి మలుపు తీసుకోండి",
    turn_uturn: "యూ-టర్న్ తీసుకోండి",
    roundabout: "రౌండ్అబౌట్‌లోకి ప్రవేశించండి",
    merge: "విలీనం చేయండి",
    fork: "ఫోర్క్ వద్ద అలాగే ఉండండి",
    "end of road": "రోడ్డు చివర కొనసాగించండి",
    arrive: "మీరు మీ గమ్యస్థానానికి చేరుకున్నారు",
...
function maneuverToText(step) {
  const phrases = MANEUVER_PHRASES[currentLang] || MANEUVER_PHRASES.en;
  const type = step.maneuver?.type || "continue";
  const modifier = step.maneuver?.modifier;
  const roadName = step.name && step.name.trim() ? step.name.trim() : "";

  let base;
  if (type === "depart") base = phrases.depart;
  else if (type === "arrive") base = phrases.arrive;
  else if (type === "roundabout" || type === "rotary") base = phrases.roundabout;
  else if (type === "merge") base = phrases.merge;
  else if (type === "fork") base = phrases.fork;
  else if (type === "end of road") base = phrases["end of road"];
  else if (type === "turn" || type === "new name" || type === "continue") {
    const key = "turn_" + (modifier ? modifier.replace(/\s+/g, "_") : "straight");
    base = phrases[key] || phrases.turn_straight;
  } else {
    base = phrases.continue;
  }

  if (roadName && type !== "arrive" && type !== "depart") {
    return `${base} ${phrases.onto} ${roadName}`;
  }
  return base;
}
...
function googleMapsDirectionsLink(fromLat, fromLng, toLat, toLng) {
...
document.getElementById("getDirectionsBtn")?.addEventListener("click", async () => {
  ...
  const resultBox = document.getElementById("directionsResult");

  if (!select.value) {
    statusEl.textContent = t("directions_pick_first");
    return;
  }
  const [, toLatStr, toLngStr] = select.value.split(":");
  const toLat = parseFloat(toLatStr);
  const toLng = parseFloat(toLngStr);

  // Get the person's current location for the starting point.
  statusEl.textContent = t("locating");
  let fromLat, fromLng;
  try {
  ...
  const gmapsUrl = googleMapsDirectionsLink(fromLat, fromLng, toLat, toLng);
  ...
  try {
    ...
    resultBox.innerHTML = `
      <h4>🧭 ${t("directions_title")}</h4>
      <div class="directions-summary">${distanceKm} km · ~${durationMin} ${t("min_eta_short")}</div>
      <ol class="directions-steps">${stepsHtml}</ol>
      <a class="directions-gmaps-link" href="${gmapsUrl}" target="_blank" rel="noopener">${t("open_in_google_maps")} ↗</a>
    `;
  } catch (err) {
    // Routing service unreachable - always fall back to Google Maps.
    resultBox.style.display = "block";
    resultBox.innerHTML = `
      <p class="directions-fallback-note">${t("directions_unavailable")}</p>
      <a class="directions-gmaps-link" href="${gmapsUrl}" target="_blank" rel="noopener">${t("open_in_google_maps")} ↗</a>
    `;
  }
});
...
function renderReportsTable(reports) {
  ...
  let html = `<table><thead><tr>
    ...
    <th>${t("th_voice")}</th><th>${t("th_status")}</th>
  </tr></thead><tbody>`;
