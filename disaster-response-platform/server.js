...
const { nearestHospitals } = require("./nearbyPlaces");
...
function nearestN(lat, lng, items, n = 3) {
  return [...items]
    .map((item) => {
...
// Nearest shelters/hospitals to a point. Hospitals prefer live OpenStreetMap
// data (accurate anywhere in the world); shelters always use the local list
// (see nearbyPlaces.js for why).
app.get("/api/nearest", async (req, res) => {
  ...
  const localHospitals = loadJSON(HOSPITALS_FILE);
  const { hospitals, source } = await nearestHospitals(lat, lng, localHospitals, 3, estimateEtaMinutes);
  res.json({
    shelters: nearestN(lat, lng, shelters, 3),
    hospitals,
    hospitalSource: source, // 'openstreetmap' | 'local'
  });
});
...
app.post("/api/reports", async (req, res) => {
  try {
    ...
    // Compute nearest help BEFORE building the report, so we can attach a
    // top-level "estimated time for help to arrive" straight onto the
    // report (used by the dashboard, the email alert, and the citizen's
    // result card) - not just buried inside the shelters/hospitals lists.
    let nearest = { shelters: [], hospitals: [], hospitalSource: "local" };
    if (finalLat != null && finalLng != null) {
      const shelters = loadJSON(SHELTERS_FILE);
      const localHospitals = loadJSON(HOSPITALS_FILE);
      const { hospitals, source } = await nearestHospitals(finalLat, finalLng, localHospitals, 3, estimateEtaMinutes);
      nearest = {
        shelters: nearestN(finalLat, finalLng, shelters, 3),
        hospitals,
        hospitalSource: source,
      };
    }
    // Prefer the nearest hospital's ETA as the "help is coming" estimate
    // (hospitals are the closest thing to an actual responder base in this
    // demo); fall back to the nearest shelter if no hospital data at all.
    const estimatedResponseEtaMinutes =
      nearest.hospitals[0]?.etaMinutes ?? nearest.shelters[0]?.etaMinutes ?? null;
    ...
    const report = {
      ...
      estimatedResponseEtaMinutes, // rough estimate only - see nearbyPlaces.js/README
      lat: finalLat,
      lng: finalLng,
    ...
    res.status(201).json({ report, nearest });
  } catch (err) {
    console.error(err);
...
app.patch("/api/reports/:id", (req, res) => {
  ...
  io.emit("update-report", reports[idx]);
  io.emit("stats-update");
